...
const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        if (force) {
            ...
            for (const jid of jids) {
                const { exists } = await signalRepository.validateSession(jid);
                if (!exists) {
        ...
        if (jidsRequiringFetch.length) {
            const lids = jidsRequiringFetch.filter(jid => (0, WABinary_1.isLidUser)(jid));
            const pns = jidsRequiringFetch.filter(jid => (0, WABinary_1.isJidUser)(jid));
            const mapped = await signalRepository.lidMapping.getLIDsForPNs(pns);
            const mappedLids = mapped ? mapped.map(m => m.lid) : [];
            const mappedPnSet = new Set(mapped ? mapped.map(m => m.pn) : []);
            const unmappedPns = pns.filter(p => !mappedPnSet.has(p));
            const wireJids = [...lids, ...mappedLids, ...unmappedPns];
    ...
    const rahmi = new SkyZee(Utils_1, waUploadToServer, relayMessage);
